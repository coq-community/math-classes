...
Section pointwise_dependent_relation. Context A (B: A → Type) (R: Π a, relation (B a)).

  Definition pointwise_dependent_relation: relation (Π a, B a) :=
    λ f f' => Π a, R _ (f a) (f' a).

  Global Instance pdr_equiv `{Π a, Equivalence (R a)}: Equivalence pointwise_dependent_relation.
...
Instance sig_equiv: Π (e: Equiv A) (P: A → Prop), Equiv (sig P) := @sig_relation.
...
Instance sigT_equiv: Π (e: Equiv A) (P: A → Type), Equiv (sigT P) := @sigT_relation.
