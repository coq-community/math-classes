...
Proof with auto.
  ...
  apply sprecedes_trans_l with (y1 + x2).
   apply (strictly_order_preserving (+ x2))...
  apply (order_preserving (y1 +))...
Qed.
...
Proof with trivial.
  ...
  split; try apply _.
  intros x y [E1 E2]. split.
   apply (order_preserving_ge_0 (.*.) z)... apply sprecedes_weaken...
  intros F. apply E2.
  apply (left_cancellation_ne_0 (.*.) z)... apply not_symmetry, neq_precedes_sprecedes...
Qed.
...
Proof with trivial. 
  ...
  rewrite <-(mult_0_r x).
  assert (GtZero x)...
  apply (strictly_order_preserving (x *.))...
Qed.
