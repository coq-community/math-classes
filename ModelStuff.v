...
Section for_signature. Variable sign: Signature.
  ...
  Section for_map.
    ...
    Proof.
     induction n; repeat intro; simpl in *.
       split; intro.
  ...
  Section for_isomorphic_implementations.
    ...
    Context (A B: sign_atomics sign -> Type)
      ...
      `{!Propers A} `{!Propers B}
      (ab: forall a, A a -> B a) (ba: forall a, B a -> A a)
      `(@HomoMorphism A B ea eb _ _ ab) `(@HomoMorphism B A eb ea _ _ ba)
    ...
    Lemma carry_eval_rev n (t: Term n) (v: Vars A): 
      @eval B _ _ (fun _ i => ab (v _ i)) t == map_op _ (@ab) (@ba) (eval v t).
    Proof with auto.
     induction t; simpl in *; intros.
       reflexivity.
      set (eval (fun (a : sign_atomics sign) (i : nat) => ab (v a i)) t2).
      pose proof (@eval_proper B eb H2 Propers1 (function (sign_atomics sign) y t1) (fun a i => ab (v a i))
        (fun a i => ab (v a i)) (reflexivity _) t2 t2 (reflexivity _): 
          Proper (eb y ==> op_type_equiv (sign_atomics sign) B t1)%signature o).
      rewrite (IHt2 v).
      subst o.
      pose proof (IHt1 v (ab (eval v t3)) (ab (eval v t3))).
      simpl in H6.
      rewrite H6.
       set (homo_proper A B (@ab)).
       set (homo_proper B A (@ba)).
       set (@map_op_proper (sign_atomics sign) A B _ _ (@ab) (@ba) p p0).
       unfold equiv.
       apply (@map_op_proper (sign_atomics sign) A B _ _ (@ab) (@ba))...
       pose proof (@eval_proper A ea H1 Propers0 _ v v (reflexivity _) t2 t2 (reflexivity _): Proper equiv (eval v t2)).
       rewrite (iso_l _ (eval v t3)).
       apply sig_type_refl...
       intro. apply _.
      reflexivity.
     pose proof (@propers A _ _ _ o).
     pose proof (@propers B _ _ _ o).
     pose proof (@preserves B A _ _ _ _ (@ba) _ o).
     revert H5 H6 H7.
     clear v.
     generalize (@op A H1 o) (@op B H2 o).
     induction (sign o).
      ...
      intros.
      rewrite <- iso_r.
      pose proof (@homo_proper A B _ _ _ _ (@ab) _ a (ba o1) o0).
      apply H8 in H7.
      assumption. (* weird *)
     ...
     apply IHo0.
       apply H5. reflexivity.
      apply H6. reflexivity.
     assert (op_type_equiv (sign_atomics sign) B o0 (o2 x) (o2 y)).
      apply H6. assumption.
     apply (@Preservation_proper B A (@eb) ea H2 H1 (@ba) H4 _ _ o0 (o2 x) (o2 y) H9 (o1 (ba y)) (o1 (ba y)))...
     change (o1 (ba y) == o1 (ba y)).
     apply H5.
     reflexivity.
    Qed. (* todo: clean up *)
...
(* todo: most of this code is copy-pasted from UniversalAlgebra.v . can we somehow
